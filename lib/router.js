...
module.exports = () => {
    app.get('/', (req, res) => {
        ...
        teams.getAll().then(data => {
            history.getTeamsRankingList().then(teamRankingList => {
                history.getPlayersRankingList().then(playerRankingList => {
                    res.render('dashboard', {teamData: data, teamRankingList: teamRankingList, playerRankingList: playerRankingList, message: query.msg});
                });
            });
        });
    });
...
app.get('/v/delete/:teamname', (req, res) => {
    res.render('deleteteam', {teamname: req.params.teamname});
});

app.get('/v/match', (req, res) => {
    teams.getAll().then(data => {
        res.render('match', {teamData: data});
    });
});
...
app.get('/p/:teamname', (req, res) => {
    teams.getByTeamName(req.params.teamname).then(teamdata => {
        history.getHistoryOfTeam(req.params.teamname).then(teamhistorydata => {
            res.render('teamprofile', {teamData: teamdata, teamHistory: teamhistorydata});
        });
    });
});
    ...
    teams.getByTeamMember(req.params.membername).then(userdata => {
        history.getHistoryOfTeam(userdata.teamname).then(teamhistorydata => {
            player.getPlayerStatistics(req.params.membername).then(playerstatistics => {
                res.render('playerprofile', {memberData: userdata, matchHistory: teamhistorydata, playerstats: playerstatistics});
            });
        });
    });
});
